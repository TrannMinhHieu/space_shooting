...
void btn_mode_callback(void* b) {
	...
	case BUTTON_SW_STATE_RELEASED: {
		...
		if(game_state == GAME_PLAY) {
			task_post_pure_msg(SST_PLAYER_SHIP_TASK_ID, SHIP_FIRE_SIG);
		} else {
...
void btn_up_callback(void* b) {
	...
	case BUTTON_SW_STATE_RELEASED: {
		...
		if(game_state == GAME_PLAY) {
			task_post_pure_msg(SST_PLAYER_SHIP_TASK_ID, SHIP_MOVE_UP_SIG);
		} else {
...
void btn_down_callback(void* b) {
	...
	case BUTTON_SW_STATE_RELEASED: {
		...
		if (game_state == GAME_PLAY) {
			task_post_pure_msg(SST_PLAYER_SHIP_TASK_ID, SHIP_MOVE_DOWN_SIG);
		} else {
